class Card {
	...
	setLikesInfo(newCardData) {
		...
		this.isLiked = this._likes.find(
			(person) => person._id === this._currentUserId
		) // null //
		if (this.isLiked) {
			this._item
				.querySelector('.gallary__icon')
				.classList.add('gallary__icon_active')
		} else {
			this._item
				.querySelector('.gallary__icon')
				.classList.remove('gallary__icon_active')
		}
	}
	...
	_getTemplate() {
		const cardElement = document
			...
			.content.querySelector('.gallary__card')
			.cloneNode(true)
	...
	generateCard() {
		...
		if (this._cardOwnerId !== this._currentUserId) {
			this._item
				.querySelector('.gallary__delete')
				.classList.add('gallary__delete_hidden')
		}
