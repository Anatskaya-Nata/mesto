...
/*function handleOpenBigImg (evt){
   popupImage.classList.add('popup_active');
const gallaryBigCards = gallaryTemplate.cloneNode(true);
   const newImg = gallaryBigCards.querySelector('.gallary__item')
   const newText = gallaryBigCards.querySelector('.gallary__text')
    newImg.src = imagePopup.value
    newText.textContent = textPopup.value
    card(textPopup.value,imagePopup.value)
evt.target.closest('.gallary__text').textContent = textPopup.value;
  evt.target.closest('.gallary__item').src = imagePopup.value;
}*/
